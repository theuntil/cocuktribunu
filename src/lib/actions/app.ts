...
export async function addChild(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  return { ok: true, message: "Çocuk kaydı eklendi." };
}
...
export async function createCardOrder(_prev: ActionState, formData: FormData): Promise<ActionState> {
  const parsed = z
    .object({
      ...
      addressId: z.string().uuid("Adres seçin"),
    })
    .safeParse({
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("create_card_order", {
  ...
  redirect(`/basvuru/odeme/${result.order_number}`);
}
...
export async function submitDonation(_prev: ActionState, formData: FormData): Promise<ActionState> {
  const parsed = z
    .object({
      amount: z.coerce.number().positive("Tutar girin").max(1_000_000),
      firstName: z.string().trim().min(2, "Adınızı girin").max(80),
      lastName: z.string().trim().min(2, "Soyadınızı girin").max(80),
      ...
      cityId: z.coerce.number().int().positive().optional().nullable(),
      visibility: z.enum(["public", "initials", "anonymous"]).default("initials"),
    ...
    .safeParse({
      ...
      contactType: formData.get("contactType") || "email",
      cityId: formData.get("cityId") || null,
      visibility: formData.get("visibility") || "initials",
  ...
  const { data, error } = await supabase.rpc("submit_donation", {
    ...
    p_donor_first_name: parsed.data.firstName,
    p_donor_last_name: parsed.data.lastName,
    p_campaign_slug: parsed.data.campaignSlug,
    p_contact_value: parsed.data.contact,
    p_contact_type: parsed.data.contactType,
    p_store_contact: formData.get("receipt") === "on",
    p_city_id: parsed.data.cityId,
    p_visibility: parsed.data.visibility,
    p_message: parsed.data.message,
    p_consent_kvkk: true,
    p_consent_terms: true,
    p_consent_contact: formData.get("receipt") === "on",
  ...
  return { ok: true, message: "Bağış kaydınız oluşturuldu.", data: data as Record<string, unknown> };
}

export async function checkDonationStatus(_prev: ActionState, formData: FormData): Promise<ActionState> {
  const parsed = z
    .object({ number: z.string().trim().min(6, "Bağış numarası girin"), token: z.string().uuid("Erişim kodu geçersiz") })
    .safeParse({ number: formData.get("number"), token: formData.get("token") });

  if (!parsed.success) return { ok: false, fieldErrors: fieldErrorsOf(parsed.error) };

  const supabase = await createClient();
  const { data, error } = await supabase.rpc("get_donation_status", {
    p_donation_number: parsed.data.number,
    p_access_token: parsed.data.token,
  });
  if (error) return { ok: false, message: friendlyError(error) };

  const result = data as { ok: boolean };
  if (!result?.ok) return { ok: false, message: "Bağış bulunamadı veya erişim kodu hatalı." };
