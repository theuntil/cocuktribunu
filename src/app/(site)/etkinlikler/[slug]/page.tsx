...
import { getEventBySlug, getEvents, getCurrentUser } from "@/lib/data";
import { createClient } from "@/lib/supabase/server";
import { formatDateLong, formatDate, EVENT_TYPE_TR, formatMoney } from "@/lib/utils";
      ...
      <div className="border-b border-line2 bg-[radial-gradient(120%_90%_at_8%_0%,var(--surface)_0%,var(--page)_46%,var(--page)_100%)]">
        <Container className="px-5 py-12 sm:px-8 lg:px-12 lg:py-16">
          <div className="ct-rise flex max-w-[780px] flex-col gap-5">
            <Link href="/etkinlikler" className="text-[13.5px] font-semibold text-green hover:underline">← Tüm etkinlikler</Link>
            <div className="flex flex-wrap gap-2">
              ...
              {event.status === "ongoing" && <Badge tone="green">Devam ediyor</Badge>}
            </div>
            ...
            <div className="flex flex-col gap-8">
                ...
                <Card className="grid gap-5 p-6 sm:grid-cols-2">
                  ...
                  {event.capacity && (
                    <Info icon={IconUsers} label="KONTENJAN"
                      value={`${event.registered_count} / ${event.capacity} · ${event.remaining_capacity ?? 0} yer kaldı`} />
                  )}
              ...
              {event.description && (
                <Reveal delay={70} className="flex flex-col gap-3">
                  <Eyebrow className="text-green">ETKİNLİK HAKKINDA</Eyebrow>
                  ...
                  <Card className="flex flex-col gap-3 p-6">
                    ...
                    {mapsUrl && (
                      <a href={mapsUrl} target="_blank" rel="noopener noreferrer"
                        className="self-start text-[14px] font-semibold text-green hover:underline">
                        Haritada aç →
                      </a>
                    )}
                  </Card>
                ...
                {event.registration_required ? (
                  user ? (
                    <EventRegistration
                      ...
                      guardianRequired={event.guardian_required}
                    ...
                    <Card className="flex flex-col gap-4 p-7">
                      ...
                      {event.requires_card && (
                        <p className="text-[13px] leading-[1.55] text-muted">
                          ...
                          <Link href="/kombine-kart" className="font-semibold text-green hover:underline">Kart nasıl alınır?</Link>
                        </p>
                      )}
                    </Card>
                  )
