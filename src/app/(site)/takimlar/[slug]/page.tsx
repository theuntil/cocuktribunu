...
import { getTeam, getTeams, getCities, getEvents, getMainCampaign, getCampaignLeaderboard } from "@/lib/data";
      ...
      <div className="border-b border-line2 bg-[radial-gradient(120%_90%_at_8%_0%,var(--surface)_0%,var(--page)_46%,var(--page)_100%)]">
        <Container className="px-5 py-12 sm:px-8 lg:px-12 lg:py-16">
          <div className="ct-rise flex flex-col gap-6">
            <Link href="/takimlar" className="text-[13.5px] font-semibold text-green hover:underline">← Tüm takımlar</Link>
        ...
        <Container>
          ...
          {teamEvents.length > 0 && (
            <Reveal delay={140} className="mt-12 flex flex-col gap-5">
              <Eyebrow className="text-green">TAKIMA ÖZEL ETKİNLİKLER</Eyebrow>
              <H2 className="text-[28px]">Yaklaşan buluşmalar</H2>
              <div className="grid gap-4 md:grid-cols-2">
                {teamEvents.map((e) => (
                  <Link key={e.id} href={`/etkinlikler/${e.slug}`}>
                    <Card className="flex flex-col gap-3 p-5 transition-colors hover:border-green">
