...
import { getMainCampaign, getCampaignLeaderboard, getTeams, getCities } from "@/lib/data";
...
export default async function SignaturePage() {
  const [campaign, teams, cities] = await Promise.all([getMainCampaign(), getTeams(), getCities()]);
  ...
  return (
    <>
      ...
      <PageHeader
        eyebrow="İMZA KAMPANYASI"
        title={campaign?.title ?? "Çocuklar Tribünde Olsun"}
        description="Her çocuğun stadyumda güvenle maç izleyebilmesi için kulüpleri ve federasyonu göreve çağırıyoruz. İmzalamak ücretsizdir ve üyelik gerektirmez."
      />
            ...
            <div className="flex flex-col gap-10">
              <Reveal>
                <Card className="flex flex-col gap-5 p-7">
                  <div className="flex flex-wrap items-end justify-between gap-4">
                    <StatBlock value={<CountUp to={count} />} label="toplanan imza" tone="green" />
                    <div className="text-right">
                      <span className="font-display text-[20px] font-semibold text-ink2">{formatNumber(target)}</span>
                      <div className="text-[13px] text-muted">hedef</div>
                    </div>
                  </div>
                  <ProgressBar value={progress} />
                  <div className="flex flex-wrap justify-between gap-2 text-[13px] text-muted">
                    <span>%{(progress ?? 0).toFixed(1)} tamamlandı</span>
                    <span>{campaign?.ends_at ? `${formatDate(campaign.ends_at)} tarihine kadar` : "Kampanya devam ediyor"}</span>
                  </div>
                </Card>
              </Reveal>

              <Reveal delay={80} className="flex flex-col gap-4">
                <Eyebrow className="text-green">NE İSTİYORUZ</Eyebrow>
                ...
                <div className="flex flex-col gap-4">
                  {[
                    ...
                    <div key={item.t} className="flex gap-4 rounded-[18px] border border-line bg-surface p-5">
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[14px] bg-green-soft text-green">
              ...
              {leaderboard.length > 0 && (
                <Reveal delay={100} className="flex flex-col gap-4">
                  <Eyebrow className="text-green">TAKIM SIRALAMASI</Eyebrow>
                        ...
                        <div key={t.team_id} className="flex items-center gap-4 px-5 py-3.5">
                          ...
                          <div className="flex min-w-0 flex-1 flex-col gap-1.5">
                            ...
                            <div className="h-1.5 w-full overflow-hidden rounded-full bg-chip">
                              <div className="h-full rounded-full bg-green transition-[width] duration-700"
                                style={{ width: `${(t.signature_count / max) * 100}%` }} />
                            </div>
                          </div>
                          <span className="shrink-0 text-[14px] font-bold text-green">{formatNumber(t.signature_count)}</span>
                        </div>
                      );
                    })}
                  </Card>
