    ...
    <PanelBody>
      ...
      {items.length === 0 ? (
        ...
        <Card className="ct-fade divide-y divide-line2 overflow-hidden">
          {items.map((n) => (
            <div key={n.id} className={`flex gap-4 px-5 py-4 sm:px-6 ${!n.read_at ? "bg-green-soft/40" : ""}`}>
              <span className={`mt-1 h-2 w-2 shrink-0 rounded-full ${n.read_at ? "bg-line" : "bg-green"}`} aria-hidden />
