    ...
    <PanelBody>
      ...
      {regs.length === 0 ? (
        <EmptyState
          ...
          action={<ButtonLink href="/etkinlikler" variant="green">Etkinlikler</ButtonLink>}
        />
      ) : (
        <>
          {upcoming.length > 0 && (
...
function RegCard({ reg, showCode }: { reg: MyEventRegistration; showCode?: boolean }) {
  ...
  return (
    <Card className="flex flex-col gap-4 p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex flex-col gap-1.5">
          <Link href={`/etkinlikler/${reg.event_slug}`}
            className="font-display text-[19px] font-semibold tracking-[-.02em] hover:text-green">
            {reg.event_title}
          </Link>
      ...
      {reg.checked_in_at && (
        <div className="flex items-center gap-2 rounded-[12px] bg-green-soft px-4 py-2.5 text-[13.5px] font-semibold text-green">
