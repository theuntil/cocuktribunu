...
export default async function Page() {
  ...
  const [pendingPayments, pendingDonations, cardsToPrint] = await Promise.all([
    supabase.from("payments").select("amount", { count: "exact" }).in("status", ["pending", "awaiting_review"]),
    supabase.from("donations").select("amount", { count: "exact" }).in("status", ["pending", "awaiting_review"]),
  ...
  const donSum = (pendingDonations.data ?? []).reduce((s, r) => s + Number((r as { amount: number }).amount), 0);

  const TILES = [
    { href: "/yonetim/odemeler", icon: IconMoney, label: "Bekleyen ödeme", count: pendingPayments.count ?? 0, extra: formatMoney(paySum) },
    { href: "/yonetim/bagislar", icon: IconHeart, label: "Bekleyen bağış", count: pendingDonations.count ?? 0, extra: formatMoney(donSum) },
