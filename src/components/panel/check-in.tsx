...
export function CheckInScanner() {
  ...
  return (
    <div className="flex max-w-[560px] flex-col gap-5">
      ...
      {state.message && (
        <Card className={`ct-scale flex flex-col items-center gap-4 p-8 text-center ${state.ok ? "border-green" : "border-danger"}`}>
          <span className={`flex h-16 w-16 items-center justify-center rounded-full ${state.ok ? "bg-green-soft text-green" : "bg-danger-soft text-danger"}`}>
          ...
          {state.ok && attendee && (
            <div className="flex flex-col gap-1">
              ...
              {typeof state.data?.attendee_count === "number" && (
                <span className="mt-1 text-[13.5px] font-semibold text-green">
