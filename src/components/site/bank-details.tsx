...
export function BankDetails({ reference, amount }: { reference: string; amount: string }) {
  ...
  return (
    <Card className="flex flex-col gap-5 p-6 sm:p-7">
      <span className="flex items-center gap-2.5 font-display text-[19px] font-semibold tracking-[-.02em]">
        <Icon icon={IconBank} size={20} className="text-green" /> Havale / EFT bilgileri
      </span>
...
function Line({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-4">
      ...
      <span className={strong ? "text-[17px] font-bold text-green" : "text-[14.5px] font-semibold text-ink"}>{value}</span>
    </div>
  );
}
