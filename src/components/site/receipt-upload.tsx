    ...
    <Card className="flex flex-col gap-4 p-6 sm:p-7">
      ...
      <label
        className={`flex cursor-pointer flex-col items-center gap-3 rounded-[18px] border-2 border-dashed px-6 py-10 text-center transition-colors ${
          path ? "border-green bg-green-soft" : "border-line bg-field hover:border-green"
      ...
      >
        ...
        />
        <span className={`flex h-12 w-12 items-center justify-center rounded-[16px] ${path ? "bg-green text-on-green" : "bg-chip text-muted"}`}>
      ...
      {path && (
        <form action={action} className="flex flex-col gap-3">
          ...
          <Button type="submit" size="lg" variant="green" loading={pending}>Dekontu gönder</Button>
        </form>
      )}
    </Card>
  );
